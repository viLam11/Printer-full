import React from "react";
import "./Dashboard.css";

const Dashboard = () => {
  return (
    <div className="dashboard">
      <div className="profile-card">
        <div className="profile-info">
          <h2>My Identify</h2>
          <p>
            <strong>Name:</strong>
          </p>
          <p>
            <strong>Student ID:</strong>
          </p>
          <p>
            <strong>Faculty:</strong>
          </p>
          <p>
            <strong>Address:</strong>
          </p>
        </div>
        <div className="profile-pic">
          <img
            src="data:image/png;base64,"
            alt="Profile"
          />
        </div>
      </div>

      <div className="recent-transactions">
        <h2>Recent Transaction</h2>
        <ul>
          <li>Deposit - 17 September 2024 - +100,000đ</li>
          <li>Print A4 paper - 15 September 2024 - -25,000đ</li>
          <li>Print A3 paper - 10 September 2024 - -30,000đ</li>
        </ul>
      </div>

      <div className="weekly-activity">
        <h2>Weekly Activity</h2>
        <div className="chart">[Bar Chart Placeholder]</div>
      </div>

      <div className="expense-statistics">
        <h2>Expense Statistics</h2>
        <div className="chart">[Pie Chart Placeholder]</div>
      </div>

      <div className="paper-remaining">
        <h2>Paper Remaining</h2>
        <div className="paper-types">
          <div>A4 paper</div>
          <div>A3 paper</div>
          <div>A2 paper</div>
        </div>
      </div>

      <div className="balance-history">
        <h2>Balance History</h2>
        <div className="chart">[Line Chart Placeholder]</div>
      </div>
    </div>
  );
};

export default Dashboard;
